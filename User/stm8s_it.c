...
INTERRUPT_HANDLER(EXTI_PORTC_IRQHandler, 5)
{
  /* In order to detect unexpected events during development,
     it is recommended to set a breakpoint on the following instruction.
  */
  if((GPIO_ReadInputData(IN_SIGNAL_PORT)&0x40)==0x40)
  {
    counter_timer=0;
    if(counter_pulse>0)
    {
      if(cnt_delay>3) counter_pulse++;
    }
    else counter_pulse++;
    flag_timer=1;
    
    //while(counter_timer<3);
    TIM2_Cmd(ENABLE);
    while((GPIO_ReadInputData(IN_SIGNAL_PORT)&0x40)==0x40);
  }
...
/**
  ...
  */
   ...
   {    
     ...
     if(counter_timer>12)
     {
       if(counter_pulse==2) // correct detect
       {
         GPIO_WriteReverse(OUT_LOAD_PORT,OUT_LOAD_PIN);
       }
       cnt_delay=0;
       counter_timer=0;
       counter_pulse=0;
